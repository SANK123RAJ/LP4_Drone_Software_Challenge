...
{
    public StructurePrefabWeighted[] housesPrefabe, specialPrefabs, BigStructuresPrefabs,ElectricHousePrefabs,SchoolPrefabs;
    public PlacementManager placementManager;

    private float[] houseWeights, specialWeights, bigStructureWeights,ElectricHouseWeights,SchoolWeights;
    ...
    private void Start()
    {
        ...
        ElectricHouseWeights = ElectricHousePrefabs.Select(prefabStats => prefabStats.weight).ToArray();
 ...
 public void PlaceSchool(Vector3Int position)
    {
        ...
        if(CheckBigStructure(position,width,height)){
            ...
            placementManager.PlaceObjectOnTheMap(position, SchoolPrefabs[randomIndex].prefab, CellType.Structure,width,height);
    ...
    public void PlaceElectricHouse(Vector3Int position)
    {
        if (CheckPositionBeforePlacement(position))
        {
            int randomIndex = GetRandomWeightedIndex(ElectricHouseWeights);
            placementManager.PlaceObjectOnTheMap(position, ElectricHousePrefabs[randomIndex].prefab, CellType.Structure);
            AudioPlayer.instance.PlayPlacementSound();
        }
    }
